...
module.exports = {
  ...
  output: {
    ...
    filename: "[name].js", // [name]と'./src/index.ts'が紐づく。[name]はentryで記述した名前（この設定ならbundle）
  },
  ...
  devServer: {
    static: {
      // webpack-dev-serverの新しいバージョンでは、contentBaseの代わりにstaticプロパティを使用する必要がある。
    ...
    rules: [
      {
        ...
        loader: "ts-loader",
        test: /\.ts$/, // 正規表現で.tsで終わるファイルをts-loaderでコンパイルする指定をする。
      },
    ],
  },
};
